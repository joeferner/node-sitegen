...
module.exports = function(options, callback) {
  ...
  options.partialsDir = path.resolve(options.partialsDir || path.join(options.indir, '_partials'));
  ...
  async.auto({
    ...
    'partials': ['config', readPartials.bind(null, options)],
    'files': ['config', 'posts', 'partials', 'outdir', processFiles.bind(null, options)],
    'rss': ['config', 'posts', 'partials', 'outdir', createRssFeedFile.bind(null, options)]
  }, callback);
};
...
function readPartials(options, callback) {
  return glob('*.ejs', { cwd: options.partialsDir }, function(err, files) {
    if (err) {
      return callback(err);
    }
    return async.forEach(files, processPartialFile.bind(null, options), function(err) {
      if (err) {
        return callback(err);
      }
      return callback();
...
function processPartialFile(options, fileName, callback) {
  var fullFileName = path.resolve(options.partialsDir, fileName);
  return readPartialFile(options, fullFileName, function(err, partial) {
    if (err) {
      return callback(err);
    }
    var name = path.basename(fileName, '.ejs');
...
function processFile(options, fileName, callback) {
  ...
  if (fullFileName.indexOf(options.layoutsDir) == 0
    || fullFileName.indexOf(options.partialsDir) == 0) {
    return callback();
  }
...
function processTemplateFile(options, fileName, callback) {
  ...
  return readTemplateFile(options, sourceFullFileName, function(err, template) {
    ...
    return readLayoutFile(options, layoutsFileName, function(err, layout) {
      ...
      var renderOpts;
      renderOpts = {
        nextPost: getNextPost(options, template),
        prevPost: getPrevPost(options, template),
        partial: function(partialName, partialOptions) {
          partialOptions = partialOptions || {};
          var partialData = options.partials[partialName];
          if(!partialData) {
            throw new Error('Unknown partial "' + partialName + '"');
          }
          var opts = {};
          merge(renderOpts, opts);
          merge(partialOptions, opts);
          return ejs.render(partialData.data, opts);
        }
      };
      ...
      var renderedData;
      try {
        renderOpts.body = ejs.render(template.body, renderOpts);
        renderedData = ejs.render(layout, renderOpts);
      } catch (ex) {
        return callback(new Error('Could not render ' + path.relative(options.indir, sourceFullFileName) + '\n' + ex.stack));
      }
...
function getPostIndex(options, post) {
  for (var i = 0; i < options.posts.length; i++) {
    if (options.posts[i].href == post.href) {
      return i;
    }
  }
  return -1;
...
function getNextPost(options, currentPost) {
  var idx = getPostIndex(options, currentPost);
  if (idx < 0 || idx >= options.posts.length - 1) {
    return null;
  }
  return options.posts[idx + 1];
}

function getPrevPost(options, currentPost) {
  var idx = getPostIndex(options, currentPost);
  if (idx <= 0) {
    return null;
  }
...
function readPartialFile(options, fileName, callback) {
  return fs.readFile(fileName, function(err, data) {
    if (err) {
      return callback(err);
    }
